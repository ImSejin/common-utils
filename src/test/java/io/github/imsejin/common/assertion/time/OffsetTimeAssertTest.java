...
class OffsetTimeAssertTest {
    ...
    class IsSameOffset {
        ...
        void test1() {
            ...
            // expect
            params.forEach(
                    offset -> assertThatIllegalArgumentException().isThrownBy(() -> Asserts.that(OffsetTime.now(offset))
        ...
        void test1() {
            ...
            params.forEach(
                    offset -> assertThatIllegalArgumentException().isThrownBy(() -> Asserts.that(OffsetTime.now(offset))
                            ...
                            .withMessageMatching(
                                    Pattern.quote(OffsetAssertable.DEFAULT_DESCRIPTION_IS_NOT_SAME_OFFSET) +
                                            "\n {4}actual: '.+'" +
                                            "\n {4}actual\\.offset: '(Z|[+-][0-9]{2}:[0-9]{2})'" +
                                            "\n {4}expected: '(Z|[+-][0-9]{2}:[0-9]{2})'"));
        }
